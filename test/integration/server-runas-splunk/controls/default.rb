...
control 'Splunk admin password validation' do
  ...
  # Run through su rather than sudo because some systemd container images have
  # incomplete sudo/PAM account data even when the splunk account is valid.
  describe command("su - splunk -c 'export HOME=#{SPLUNK_HOME} && #{SPLUNK_HOME}/bin/splunk login -auth admin:notarealpassword'") do
    its('exit_status') { should eq 0 }
  end
end
